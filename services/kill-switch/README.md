...
| The endpoint call cannot be steered off the allowlist (no SSRF) | **Enforced in code** | exact `urlsplit(...).hostname` match; scheme restricted to http/https; tests that fail if the comparison is weakened to a substring or suffix test |
...
| A halt signal never recurses into this service | **Enforced in code** | `x-field-kill-origin` header short-circuit **and** the `/kill/{agent}` / `/kill/domain/` path skip; a test for each |
