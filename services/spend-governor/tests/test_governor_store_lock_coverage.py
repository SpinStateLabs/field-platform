"""Every use of GovernorStore's shared sqlite3 connection is inside ``with self._lock``.

``spend_governor.core.GovernorStore`` keeps ONE sqlite3 connection shared by every
...
(tests/test_governor_store_concurrency.py) catches unlocked READS, statistically. It did not catch
