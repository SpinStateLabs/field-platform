"""Every use of RegistryStore's shared sqlite3 connection is inside ``with self._lock``.

``agent_registry.store.RegistryStore`` keeps ONE sqlite3 connection shared by every
...
(tests/test_registry_store_concurrency.py) catches unlocked READS, statistically. It did not catch
