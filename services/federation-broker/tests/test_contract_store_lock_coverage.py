"""Every use of ContractStore's shared sqlite3 connection is inside ``with self._lock``.

``federation_broker.engine.ContractStore`` keeps ONE sqlite3 connection shared by every
...
(tests/test_contract_store_concurrency.py) catches unlocked READS, statistically. It did not catch
...
MODULE = "federation_broker.engine"
CLASS = "ContractStore"
CONN = "_conn"
LOCK = "_lock"
