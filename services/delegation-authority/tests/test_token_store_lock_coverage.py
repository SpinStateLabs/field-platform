"""Every use of TokenStore's shared sqlite3 connection is inside ``with self._lock``.

``delegation_authority.store.TokenStore`` keeps ONE sqlite3 connection shared by every
...
(tests/test_token_store_concurrency.py) catches unlocked READS, statistically. It did not catch
...
MODULE = "delegation_authority.store"
CLASS = "TokenStore"
CONN = "_conn"
LOCK = "_lock"
